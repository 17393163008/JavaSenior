 ...
 *                |---ArrayList：作为List的主要实现类，线程不安全，效率高；底层使用Object[]存储 elementData存储
 ...
 *      ...
 *      list.add(123);//如果此次添加导致底层elementData数组容量不够，则扩容
 ...
 *
 *     2.3 小结：
 *      jdk7中ArrayList的创建类似于单例的饿汉式，而jdk8中的ArrayList对象创建类似于懒汉式
 *      延迟了数组的创建，节省内存
 *
 *   3、LinkedList源码分析
 *      LinkedList lsit = new LinkedList(); 内部声明了Node类型的first和last
 *      list.add(123); //将123封装到Node中，创建了Node对象。
 *
 *      其中，Node定义了：体现了LinkedList的双向链表的说法
 ...
 *     }
 *
 *   4、Vector源码分析：jdk7和jdk8中通过Vector()构造器创建对象时，底层创建了长度为10的数组
...
public class ListTest {
    ...
    public void test2(){
        ...
        //int indexOf(Object obj):返回obj首次出现的位置，如果不存在，返回-1
        ...
        //Object remove(int index):移除指定index未知的元素，并返回此元素
